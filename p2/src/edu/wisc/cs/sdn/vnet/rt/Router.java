...
{	
	...
	public void handlePacket(Ethernet etherPacket, Iface inIface)
	{
		...
		short checksum = ipv4.getChecksum();
		ipv4.resetChecksum();
		ipv4.serialize();
		if(checksum != ipv4.getChecksum()){
		...
		for(Iface iface : this.interfaces.values()){
			if(iface.getIpAddress() == ipv4.getDestinationAddress()){
				return;
			}
		}

		RouteEntry re = routeTable.lookup(ipv4.getDestinationAddress());
		if(re == null){
			System.out.println("Route Entry Not Found!");
			return;
		}
		int nextHopIp;
		if(re.getGatewayAddress() == 0){
			nextHopIp = ipv4.getDestinationAddress();
		} else {
			nextHopIp = re.getGatewayAddress();
		}
